...
After noticing, the pricing kernel is actually $e^{-r \tau}$ mutilpying a normal distribution, we can define $d_{-}(\tau, x) = \frac{1}{\sigma \tau} [\frac{x}{lnK} + (r - \frac{1}{2}\sigma^2)\tau], d_{+} = d_{-} + \sigma \sqrt{\tau}$, $N(x) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^x e^{-\frac{x^2}{2}}$. We separate the integration into two parts: $K$ and $e^x$.\par
\begin{subequations}
\begin{align}
