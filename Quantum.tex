...
Define the time-evolution operator $\widehat{U(t)}$ for state vector $\ket{\psi(t)}$ to be the operator such that:
\begin{equation}
\ket{\psi(t)} = \widehat{U(t)}\ket{\psi(0)}
\end{equation}
\indent Intuitively, time-evolution operator is the coefficient matrix to express $\psi(t)$ in terms of eigenfunctions.\par

Define the propagator $K(x'', t; x', t_0)$ such that $\psi(x'',t) = \int{d^3x' K(x'',t;x',t_0)\psi(x', t_0)}$, where it is given by
\begin{equation}
K(x'',t;x',t_0) = \sum_{a'}\braket{x''|a'}\braket{a'|x'}e^{-iE_{a'}(t - t_0)/\hbar}
\end{equation}
